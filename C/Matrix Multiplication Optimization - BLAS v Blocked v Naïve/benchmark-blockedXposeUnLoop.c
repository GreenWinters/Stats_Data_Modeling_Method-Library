...
int main (int argc, char **argv)
{
  ...
  if (nmax%2 != 0) // if the matrix sizes are not even, make them even
        nmax += 1;
  ...
  for (int isize = 0; isize < sizeof(test_sizes)/sizeof(test_sizes[0]); ++isize)
  {
    ...
    double* A_Transpose = C + nmax*nmax;

    fill (A, n*n);
    fill (B, n*n);
    fill (C, n*n);
    fill (A_Transpose, n*n);

/* Transpose A to Row-major order */
    for (int row = 0; row < n; ++row)
        for (int col = 0; col < n; ++col) A_Transpose[col+row*n] = A[row+col*n];
    ...
    {
      /* Warm-up */
      square_dgemm (n, A_Transpose, B, C);
